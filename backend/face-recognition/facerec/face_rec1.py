...
class Config(object):
    ...
    emb_file='/home/vinoo/Experiments/embeddings/enroll.npz'
    enroll_file1='/home/vinoo/Experiments/embeddings/enroll.npz'
    enroll_file='/home/vinoo/Experiments/embeddings/enroll1.npz'
...
class facerec(object):
    ...
    def detect_faces(self,image_path):
        ...
        threshold = [ 0.99, 0.99, 0.99 ]  # [0.6,0.7,0.7] three steps's threshold
...
# so that loading happens once
        if not self.args.enroll_is_same:
            embeddings1,paths1=self.load_enroll_embeddings(self.args)
            self.embeddings=np.vstack([self.embeddings, embeddings1])
            self.paths=np.hstack([self.paths, paths1])

        if self.args.do_cluster:
            npaths,nembs=clus_emb.foo1(self.embeddings,self.paths)
            self.cembeddings=nembs
            self.cpaths=npaths
    ...
    def get_match2(self,embeddings,paths,emb_test,args=None):
        if self.args.do_cluster:
            npaths,nembs=self.cpaths,self.cembeddings
        else:
            #npaths,nembs=clus_emb.foo1(embeddings,paths)
	     npaths,nembs=self.paths,self.embeddings
        npaths=np.asarray(npaths)
        #print (len(npaths),len(nembs),nembs.shape)
       # t_nembs = tf.Variable(nembs, dtype=tf.float64)
       # t_emb_test = tf.Variable(emb_test, dtype=tf.float64)
        print("------------------")
       # print (t_nembs.get_shape())
       # print(t_emb_test.get_shape())
       # t_diff = tf.subtract(t_nembs, t_emb_test)
   
       # t_dist = tf.reduce_sum(tf.square(t_diff), 1)
       # print (t_dist.get_shape())
        print ("-----------------------")
       # print(t_dist)
#        t_idx_sort = tf.argmin(t_dist)
      #  with tf.Session() as sess:
      #      sess.run(t_nembs.initializer)
      #      sess.run(t_emb_test.initializer)
      #      print("==========================================")
      #      print (t_diff.eval(sess))
      #      print ("==========================lllllllllllllllll")
      #      te_dist = t_dist.eval()
      #      print (te_dist)
      #      idx_sort = np.argmin([t_dist.eval()])
            #idx_sort = np.array([t_idx_sort.eval()])
        diff = np.subtract(nembs, emb_test)
        dist = np.sum(np.square(diff),1)
...
##        idx_sort=np.argsort(dist)[0:ntop]
        ...
        #print (idx_sort,npaths[idx_sort])
        return npaths[idx_sort],dist[idx_sort]
    ...
    def recognize(self,args,inp):
        st=time.time()
#PS - commenting below - not needed now as we are loading embeddings with enrol at start
 #       if not self.args.do_cluster:
#            self.load_embeddings()
        ...
        for emb_test,aligned in nfaces:
            #emb_test=self.get_face_embedding(whitened,args)
            match_paths,dists=self.get_match2(embeddings,paths,emb_test,args)
            #merged=self.merge_imgs(match_paths,aligned)
            print (match_paths,dists)
            #if dists[0] > 1.0 : continue
            tmp_path=[path_.split('/')[-2] for path_ in match_paths]
            ret_names.append(tmp_path[0] + " ("+str(dists[0])+")")
            ret_path=self.write_eval_one_img(aligned,tmp_path[0]  + " ("+str(dists[0])+")")
...
#PS - load to in meory object
        self.embeddings = np.vstack([self.embeddings,emb_test])
        self.paths = np.hstack([self.paths,[new_path]])

    #    np.savez(args.enroll_file1,embedding=np.vstack([emb_test]),paths=np.hstack([[new_path]]))
        return emb_test
