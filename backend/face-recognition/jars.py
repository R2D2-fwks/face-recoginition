...
# Created by vinoth for enrolling celebs from santabanta.com
ENROLL_FILE_FOLDER = './enrolled_image'
@app.route('/trainenroll', methods=['POST'])
def trainenroll():
      try:
            print("******************* received a request *********************")
            name = request.form['na']
            url = request.form['url']
            filename = request.form['save_as']
            print "name: ", name
            print "url: ", url
            ...
            with open(os.path.join(ENROLL_FILE_FOLDER, filename), 'w') as f:
                  f.write(result.read())
